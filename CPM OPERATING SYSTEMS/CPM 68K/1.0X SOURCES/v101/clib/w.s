...
.globl  _sw
		.text
__start:
		clr.l	d5			* 	clear a register
		jsr	_sw			* 	security reasons
		bra	start			*	Branch around ID
		.dc.b 'C Runtime V01.10W, Copyright(c) 1983, Digital Research '
...
xclear:						*	Clear heap area
		...
		move.l  #$20,d4			*	20 extra bytes
		adda.l  d4,a1			*	make extra room past bss
		bsr	_even			*	assure even address
		move.l	d0,__break		*	Put in "break" loc
		move.l	d0,_nextarg		*	place to put argvs
		...
		move.l	sp,_lastarg		*	last spot to put an arg
		subi.l	#$100,_lastarg		*	little extra space from stack
...
noovf:						*	Here if all OK
		...
		.globl _make_even
_make_even:
		movea.l	4(sp),a1		*	make this address even
_even:		move.l 	a1,d5			*	save it
		move.l	a1,d0			*	prepare to return it
		andi.l	#$1,d5			*	check for even address
		bne	odd			*
		rts				*	return it 
odd:		addq	#$1,d0			*	make it even
		rts				*	return it
...
*
		...
		.globl  _nextarg		*	next place to put arg
		.globl	_lastarg		*	last place to put arg
...
___xeof:	.dc.b	$1a			*	Control-Z
		.even
		.bss
_nextarg:	.ds.l	1			*	ptr to next argv start
		.even
_lastarg:	.ds.l	1			*	ptr to last argv start
		.ds.l	1
		.end
