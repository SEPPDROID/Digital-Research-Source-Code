...
match(b)					/*   (used in transferring  */
int	b;					/*   that part of a file    */
{						/*   between delimiter pair)*/
	int	c;				/* b indexes start of match */
						/*   string in command buff */
...
{						/*   or from start match    */
	...
	{					/*   (if user specified one)*/
	    ...
	    {
		if (match(STARTS))		/* Complete match?	    */
...
{						/*   end of line	    */
						/* Also translates buffer to*/
						/*   upper case		    */
	return ((++combuf.cbp >= (combuf.comlen & 0xff)) ?
		CR : (combuf.comline[combuf.cbp] =
		       utran(combuf.comline[combuf.cbp])));
}
...
struct flctlb *fcba;				/*   brackets		    */
{
	      ...
	      && (ch != ']'))
	{
	    ...
	    switch (ch)				/* Check for modifiers after*/
	    {					/*   option		    */
	      case 'D':				/* Delete past column n	    */
	      case 'P':				/* Page length		    */
	      case 'T':				/* Tab width		    */
		cont[ch - 'A'] = a_to_i();	/* Plug in value (note lack */
...
{						/*   return index in io to  */
	...
	for (i = 0; i < TYPES; i++)		/* For each valid name	    */
	{
	    ...
	    {
		if (io[i] [j] != combuf.comline[combuf.cbp - 3 + j])
...
struct flctlb *fcba;				/* Set up any options       */
{
	...
	*fcba = empty_fcb;			/* Clear out FCB	    */
	...
	{					/* Yes.  How long was it?   */
		...
		if (! ('A' <= (ch = combuf.comline[combuf.cbp - 1])
